...
Everything else in Part I is combinations and elaborations of those six. Chapter 2 goes deeper on prompts and structured output. Chapters 3 and 4 build a RAG pipeline. Chapter 5 covers tool-calling in more depth. Chapters 6 through 10 introduce LangGraph and use it to build stateful, durable, human-in-the-loop agents. If any of the primitives in this chapter feels shaky, run the example, tweak it, break it — the feedback loop with a local model is fast enough that experimenting is basically free.
