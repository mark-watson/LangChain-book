...
- **Tool calling** (Chapters 7 and 19) returns tool call requests that you then execute. Use it when the LLM's job is to *decide* to do something that produces data.
...
Chapter 22 wraps up Part II by deploying a workflow as a service using `llama-deploy` — Redis + Python, no cloud.
